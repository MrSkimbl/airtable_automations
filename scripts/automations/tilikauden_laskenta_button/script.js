// Automation: "Tilikauden laskenta"  (wflKTRdcFMFSH8j6H)
// Status: deployed
// Trigger: When a button is clicked (interface "Bonuslaskenta")
// Toiminto: aggregoi Asiantuntijat-taulun rivit Tilikauden laskenta -tauluun.
//
// EROAA scheduled-versiosta:
//  - hyväksyy useita asiantuntijoita per rivi (ottaa ensimmäisen) — scheduled vaatii length===1
//  - mukana mrrAsiakkaatCounts (uniikit MRR-asiakkaat per kk)
//  - Lähtevät: jos Laskutusennuste < 100€, käytä 5000€
...
let mrrAsiakkaatCounts = {};
let expertIds = {};

for (let rec of srcQuery.records) {
    let expertsCell = rec.getCellValue("Asiantuntija");
    if (!Array.isArray(expertsCell) || expertsCell.length === 0) continue;

    // Jos useita asiantuntijoita, käsittele vain ensimmäinen
    ...
    // MRR-asiakkaiden lukumäärä per kk
    if (category==="MRR" && nbeb!=="NB" && total>0) {
        for (let m of months) {
            let val = safeNumber(rec.getCellValue(m));
            if (val > 0) {
                let key = `${expertName}__${m}`;
                mrrAsiakkaatCounts[key] = (mrrAsiakkaatCounts[key]||0) + 1;
            }
        }
    }
    ...
    if (category === "Komissio") {
        ...
        if (["Tulossa", "Työn alla"].includes(status)) {
            ...
            if (months.includes(monthName)) {
                ...
                // Jos Laskutusennuste 0 tai lähellä sitä (< 100€), aseta 5000€
                if (sumVal < 100) sumVal = 5000;
                let key = `${expertName}__${monthName}`;
...
for (let rec of dstQuery.records) {
    ...
    let expertRef = rec.getCellValue("Asiantuntija");
    let expertName = (Array.isArray(expertRef) && expertRef.length > 0) ? safeString(expertRef[0]) : "";
...
const allKeys = new Set([
    ...
    ...Object.keys(mrrAsiakkaatCounts)
]);
...
for (let key of allExistingKeys) {
    ...
    let fieldsToUpdate = {
        ...
        "MRR asiakkaita, kpl": mrrAsiakkaatCounts[key] || 0,
        "Tilikausi": "2025–2026"
    };
